...
| Lifecycle | `core/loop.ts`, `game.ts` | `GameLoop.hold(reason, on)` for system pauses (hidden tab, pause sheet, rotate). Touch devices do not run the hidden-tab timer; the harness (`?nolock`) still does. Saves on `visibilitychange` and `pagehide`. Audio unlock runs from the canvas pointerdown and the title button gesture. |
...
| Settings | `persistence/save.ts`, `ui/title.ts` | `quality` (auto/low/high, applies on the next load) and `look` (0.5–2×). The settings modal doubles as the pause sheet (`TitleScreen.openSettings`) with a mute button. |
| Responsive CSS | `ui/hud.css`, `ui/title.css` | `--u` floors at 8 px; safe-area insets; the touch HUD layout; phone inventory as GEAR / BAG / SKILLS tabs with an 8×5 bag and a bottom-sheet item card (Equip / Drop / Unequip); title and select re-flow under 900 px wide or 520 px tall, and stack into one column on portrait phones. |
...
- Death card taps to respawn; pause sheet holds the sim, saves look and quality, resumes.
